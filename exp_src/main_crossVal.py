...
model_name = 'resnext101_64x4d_crossVal'
    ...
    def train(self, args, train_loader, val_loader, lr=1e-3, ckpt_dir='ckpt'):
        ...
        for epoch in range(args.epochs):
            ...
            if (epoch + 1) % args.freq_save_ckpt == 0:
                checkpoint_path = f'{ckpt_dir}/{model_name}_epoch' \
                                  f'{epoch + 1}.pth'
                torch.save({
                    'epoch': epoch,
...
def cross_validate(args, dataset, num_folds=5, lr=1e-4, ckpt_dir='ckpt'):
...
def ensemble_validate(args, dataloader, ckpt_paths):
    ...
    acc = 0.0
    with torch.no_grad():
        for img_path, label in tqdm.tqdm(dataloader):
            img_path = img_path[0]
            img = Image.open(img_path).convert('RGB')
...
def ensemble_test(args, test_loader, ckpt_paths, output_zip='solution.zip'):
    models = [ResNeXt101_64x4d(model_name) for ckpt_path in ckpt_paths]
    for model, ckpt_path in zip(models, ckpt_paths):
        model.load_checkpoint(ckpt_path)
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        if args.ckpt:
            ...
            best_ckpt = f'ckpt/fold1/{model_name}_best.pth'
            last_ckpt = get_latest_checkpoint('ckpt/fold1', model_name)
            print("Testing with best checkpoint...")
            model.test(
                test_loader, best_ckpt,
                args.output_zip.replace('.zip', '_best.zip')
            )
            print("Testing with last checkpoint...")
            model.test(
                test_loader, last_ckpt,
        ...
        cross_validate(
            args, all_dataset, num_folds=5, lr=1e-4, ckpt_dir='ckpt'
        )
        ...
        best_ckpts = [
            f'ckpt/{model_name}_fold{i+1}_best.pth' for i in range(5)
        ]
        ensemble_test(
            args, test_loader, best_ckpts,
            num_folds=5, output_zip=args.output_zip
        )
