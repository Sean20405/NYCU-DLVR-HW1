...
model_name = 'resnext101_64x4d_focal'
...
class ResNeXt101_64x4d:
    ...
    def train(self, args, train_loader, val_loader, epochs=10, lr=1e-3,
              ckpt_dir='ckpt'):
        """
        Command: python main.py --train --device=0
        """
        ...
        self.writer.add_text(
            'Parameters',
            f'Learning Rate: {lr}, Epochs: {epochs}, '
    ...
    def test(self, test_loader, ckpt_path, output_zip='solution.zip'):
        ...
        df.to_csv(f'{model_name}/prediction.csv', index=False)
        with zipfile.ZipFile(f'{model_name}/{output_zip}', 'w') as zf:
            zf.write(f'{model_name}/prediction.csv')
        shutil.move(f'{model_name}/{output_zip}', output_zip)
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        model.train(
            args, train_loader, val_loader, epochs=50, lr=1e-4, ckpt_dir='ckpt'
        )
    if args.test:
        # Load test dataset
