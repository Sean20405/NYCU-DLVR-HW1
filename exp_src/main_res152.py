...
model_name = 'resnet152_adamw_sched'
...
class ResNet152:
    ...
    def train(self, args, train_loader, val_loader,
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % 20 == 0:
                checkpoint_path = (
                    f'{ckpt_dir}/{self.model_name}_epoch{epoch + 1}.pth'
                )
                torch.save({
                    'epoch': epoch,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--output_zip', type=str, default=f'result/{model_name}.zip',
    ...
    if args.train:
        ...
        model = ResNet152(model_name)
        model.train(args, train_loader, val_loader,
                    epochs=100, lr=1e-3, ckpt_dir='ckpt')
    elif args.test:
        model = ResNet152(model_name)
